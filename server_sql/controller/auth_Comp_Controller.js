      ...
      bcrypt.genSalt(10, (err, salt) => {
        bcrypt.hash(password, salt, (err, hash) => {
          ...
          company
            ...
            .catch((err) => {
              res.status(400).json({ err: "This username is already used" });
            });
...
exports.login_comp = (req, res) => {
  ...
  sequelize
    ...
    .then(() => {
      company
        .findOne({ where: { c_uname: String(username).toLowerCase() } })
        .then((user) => {
          if (user) {
            bcrypt
              ...
              .then((passwordCheck) => {
                if (!passwordCheck) {
                  return res
                    .status(400)
                    .json({ err: "Password doesn't match" });
                }
...
exports.accountInfo = async (req, res) => {
  ...
  try {
    const data = await company.findByPk(accountID, {
      attributes: { exclude: ["c_pass"] },
    });
    if (!data) {
